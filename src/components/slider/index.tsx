...
const Rect = ({ height, width, x, y }: { height: number; width: number; x: number; y: number }) => ({
  containsPoint: (nativeX: number, nativeY: number) => nativeX >= x && nativeY >= y && nativeX <= x + width && nativeY <= y + height,
  height,
...
export class Slider extends PureComponent<SliderProps, SliderState> {
  constructor(props: SliderProps) {
    ...
    this.state = {
      ...
      values: updateValues({
        values: normalizeValue(this.props, this.props.value instanceof Animated.Value ? this.props.value.__getValue() : this.props.value),
      }),
    };
  }
  ...
  componentDidUpdate() {
    const newValues = normalizeValue(this.props, this.props.value instanceof Animated.Value ? this.props.value.__getValue() : this.props.value);
  ...
  _renderDebugThumbTouchRect = (thumbLeft: Animated.AnimatedInterpolation, index: number) => {
    ...
    return <Animated.View key={`debug-thumb-${index}`} pointerEvents="none" style={[styles.debugThumbTouchArea, positionStyle]} />;
  };
  ...
  _renderThumbImage = (thumbIndex: number = 0) => {
    ...
    return <Image source={(Array.isArray(thumbImage) ? thumbImage[thumbIndex] : thumbImage) as ImageSourcePropType} />;
  };
  ...
  render() {
    ...
    const interpolatedThumbValues = values.map(value =>
      value.interpolate({
        ...
        outputRange: I18nManager.isRTL ? [0, -(containerSize.width - rightPadding)] : [0, containerSize.width - rightPadding],
      }),
    );
    ...
    return (
      <>
        ...
        <View {...other} style={[styles.container, vertical ? { transform: [{ rotate: '-90deg' }] } : {}, containerStyle]} onLayout={this._measureContainer}>
          <View
          ...
          <Animated.View renderToHardwareTextureAndroid style={[styles.track, trackStyle, minimumTrackStyle, propMinimumTrackStyle]}>
          ...
          {interpolatedThumbValues.map((value, i) => (
            ...
            >
              {renderThumbComponent ? (Array.isArray(renderThumbComponent) ? renderThumbComponent[i](i) : renderThumbComponent(i)) : this._renderThumbImage(i)}
            </Animated.View>
          ))}
          <View style={[styles.touchArea, touchOverflowStyle]} {...this._panResponder.panHandlers}>
            {!!debugTouchArea && interpolatedThumbValues.map((value, i) => this._renderDebugThumbTouchRect(value, i))}
          </View>
        </View>
        {renderBelowThumbComponent && (
